    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success == true) {
                ...
                $("#_totalOrders").html(totalData.orders);

                for(let i = 1 ; i <= 12 ; i++) {
                    if(data.earningsMonth[i] != null)
                        areaChartData[i - 1] = data.earningsMonth[i];
                    else
                        areaChartData[i - 1] = 0;
                }
...
function showAreaChart() {
    var myLineChart = new Chart(document.getElementById("earningsByMonth"), {
        type: 'line',
        data: {
            labels: ["Styczeń", "Luty", "Marzec", "Kwiewień", "Maj", "Czerwiec", "Lipiec", "Sierpień", "Wrzesień", "Październik", "Listopad", "Grudzień"],
